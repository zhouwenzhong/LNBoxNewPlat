package com.lianyao.ftfbox.util.http;
...
public class RestRequest {
	...
	public void setRestPost(final RestInterface restInterface,
		...
		try {
			requestParams.setBodyEntity(new StringEntity(param.toString(), "UTF-8"));
		} catch (Exception e) {
			e.printStackTrace();
		}
		...
		http.send(HttpRequest.HttpMethod.POST, reqStr, requestParams,
				new RequestCallBack<String>() {
					...
					public void onSuccess(ResponseInfo<String> responseInfo) {
						try {
							restInterface.onSuccess(JSONObject.parseObject(responseInfo.result));
						} catch(Exception e) {
